...
**Status**: Phase 18 Complete - Egyptian Arabic Display Fix

---

## Critical: Egyptian Arabic Display Solution (FINAL)

**Problem**: Lookup modal was showing MSA (Modern Standard Arabic) first instead of Egyptian Arabic, with missing letter breakdown and vowel markers.

**Root Cause**:
- LookupView used `WordDetailCard` instead of the unified `WordDisplay` component
- Egyptian dictionary lookup was not integrated into `lookupWord` function
- Wrong letter breakdown function - `generateArabicBreakdownByWord` (simple) instead of `generateArabicBreakdown` (with vowels)
- Extra wrapper div causing layout inconsistencies vs exercise mode

**Complete Solution** (January 7, 2026):

### 1. Unified Component Usage
**File**: `src/features/lookup/LookupView.tsx`
- Replaced `WordDetailCard` with `WordDisplay` component (matches lessons/exercises)
- Removed extra `glass-card` wrapper div for consistent spacing
- Passes complete word data structure to WordDisplay
...
### 2. Egyptian Dictionary Integration
**File**: `src/lib/openai.ts`
- After AI returns MSA word, override with Egyptian dictionary entry
- Generates letter breakdown from Egyptian word (not MSA)
...
if (result.arabic_word) {
  ...
  if (egyptianEquivalent) {
    ...
    // Generate letter breakdown for Egyptian word
    const egyptianBreakdown = generateArabicBreakdownByWord(egyptianEquivalent.egyptian);
    if (egyptianBreakdown && egyptianBreakdown.length > 0) {
      result.letter_breakdown = egyptianBreakdown.flatMap(wb => wb.letters);
    }
  }
}
```

### 3. Letter Breakdown with Vowels
**File**: `src/components/WordDisplay.tsx`
- Switched from `generateArabicBreakdownByWord` to `generateArabicBreakdown`
- `generateArabicBreakdown` (from `arabicLetters.ts`) combines letters with vowel diacritics
- Shows "Ya + Fatha" instead of just "ya"
- Handles multi-word phrases by splitting and processing each word

```typescript
// Import the better breakdown function
import { generateArabicBreakdown } from '../utils/arabicLetters';

// Generate letter breakdown with vowels - split multi-word phrases
const letterBreakdown = word.letterBreakdown || 
  (showLetterBreakdown && primaryArabic ? primaryArabic.split(' ').map(w => ({
    word: w,
    letters: generateArabicBreakdown(w)
  })) : []);
```

### 4. Type Structure (CRITICAL REFERENCE)

**WordBreakdown Interface** (used by WordDisplay):
```typescript
// Each word has its letters array
...
interface LetterBreakdown {
  letter: string;      // Arabic character + vowels (e.g., "يَ")
  name: string;        // Letter name + vowels (e.g., "Ya + Fatha")
  sound: string;       // Pronunciation (e.g., "ya")
  type?: 'letter' | 'diacritic';
}
```

**ArabicWordData Interface** (WordDisplay props):
```typescript
interface ArabicWordData {
  arabic: string;                    // MSA word
  arabicEgyptian?: string;          // Egyptian word (prioritized)
  translation: string;
  transliteration?: string;         // MSA transliteration
  transliterationEgyptian?: string; // Egyptian transliteration (prioritized)
  hebrewCognate?: HebrewCognate;
  exampleSentences?: ExampleSentence[];
  letterBreakdown?: WordBreakdown[]; // Array of word breakdowns
}
```

### 5. Letter Breakdown Functions (DO NOT CONFUSE)

**Use `generateArabicBreakdown`** (from `src/utils/arabicLetters.ts`):
- ✅ Combines letters with vowel diacritics
- ✅ Shows "Ya + Fatha", "Ba + Kasra", etc.
- ✅ Used in: WordDisplay, ExerciseFeedback
- Returns: `LetterBreakdown[]` for a single word

**Do NOT use `generateArabicBreakdownByWord`** (from `src/utils/arabicBreakdown.ts`):
- ❌ Simple letter-only breakdown (no vowel names)
- ❌ Less informative for learning
- Returns: `WordBreakdown[]` (array of words)

### 6. Display Order (Matches Lessons/Exercises)
1. Egyptian word (text-5xl, prominent)
2. Egyptian transliteration (text-2xl amber, highlighted)
3. MSA transliteration (text-sm white/50, reference only)
4. Hebrew cognate (smaller, below transliterations)
5. Letter breakdown (text-4xl letters with vowel names)
6. Example sentences (text-3xl Arabic, text-lg transliteration)

### Key Files & Their Roles

**Core Display**:
- `src/components/WordDisplay.tsx` - Unified word display (lessons, exercises, lookup)
- `src/features/lookup/LookupView.tsx` - Lookup mode page
- `src/features/exercises/ExerciseFeedback.tsx` - Exercise feedback display

**Data & Utilities**:
- `src/lib/openai.ts` - API calls, Egyptian dictionary integration
- `src/utils/egyptianDictionary.ts` - Egyptian ↔ MSA static mappings
- `src/utils/arabicLetters.ts` - Letter breakdown WITH vowels (USE THIS)
- `src/utils/arabicBreakdown.ts` - Simple breakdown (DO NOT USE)
- `src/utils/hebrewCognates.ts` - Hebrew cognate static mappings

**Still Used (Different Context)**:
- `src/components/WordDetailCard.tsx` - Used ONLY in MyVocabularyView for saved word modals
- Different from WordDisplay - fetches data if not provided
- Should eventually be replaced with WordDisplay for consistency

### Testing Coverage
- ✅ "car" → العَرَبِيَّة with vowel breakdown (ع + فَتحَة, etc.)
- ✅ "my car" → عَرَبِيَّتِي with possessive form
- ✅ Multi-word phrases → word-by-word breakdown
- ✅ Font sizes match exercise mode
- ✅ Spacing/layout consistent with lessons
